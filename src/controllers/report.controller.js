...
class ReportController {
  ...
  async summary(req, res) {
    try {
      ...
      const totalOrderpsLastWeeek  = Number.parseFloat(totalOrders7DaysAgo.total / totalOrders14DaysAgo.total).toFixed(2) * 100 - 100;
      const totalOrderCountpsLastWeeek  = Number.parseFloat(totalOrders7DaysAgo.count / totalOrders14DaysAgo.count).toFixed(2) * 100 - 100;
      
      response.data.revenue.psLastWeek = (!isFinite(totalOrderpsLastWeeek)) ? 100 : totalOrderpsLastWeeek;
      response.data.orders.psLastWeek = (!isFinite(totalOrderCountpsLastWeeek)) ? 100 : totalOrderCountpsLastWeeek; 
      ...
      const psLastWeek = Number.parseFloat(totalUsers7DaysAgo / totalUsers14DaysAgo).toFixed(2) * 100 - 100;
  ...
  async revenueByMonth(req, res) {
    try {
      ...
      for (const id in orders) {
        ...
        const orderMonth = (orderCreated.getMonth() + 1).toString().padStart(2, '0');
